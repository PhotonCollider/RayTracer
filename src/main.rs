...
use hittable::{ConstantMedium, Hittable, HittableList, RotateY, Translate};
use material::{Dielectric, DiffuseLight, Lambertian, Material, Metal};
use quad::{box_from_vec, Quad};
use sphere::Sphere;
use texture::{CheckerTexture, ImageTexture, NoiseTexture};
use util::{random_f64_0_1, random_f64_ranged, random_positive_vec3, random_positive_vec3_ranged};
use vec3::Vec3;
...
fn checkered_spheres() -> (Camera, HittableList) {
    let mut world = HittableList::new();
    let checker = Arc::from(CheckerTexture::from_color(
        0.32,
        Vec3::new(0.2, 0.3, 0.1),
...
fn cornell_smoke() -> (Camera, HittableList) {
    let mut world = HittableList::new();
...
fn final_scene(image_width: u32, sample_per_pixel: u32, max_depth: u32) -> (Camera, HittableList) {
...
fn main() {
    ...
    let path = "output/ssaa_test/900with_ssaa.png";

    let _jpeg_quality = 60;

    // 10k spp
    // 800 10k 40
    let (mut cam, world) = final_scene(400, 900, 40);
    cam.enable_ssaa = false;
